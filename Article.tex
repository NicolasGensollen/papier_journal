...
Clearly, as $ \alpha $ impacts the sizes of the coalitions, it should not be selected randomly. In order to have an initial guess, we use a mean field approximation in a gaussian case (as in section \ref{sec:notations}). In this section, $ \bar{x} $ denote the mean approximation of quantity x. Recall that we have N agents and we wish to form $ N_{COAL} $ coalitions. We denote by $ \bar{N} = \lfloor \dfrac{N}{N_{COAL}} \rfloor $ the expected mean number of agents in a coalition. If $ \mathcal{U}(S) $ favors coalitions of size $ \bar{N} $, then :

\begin{equation}
\left[ \dfrac{\partial{ U}}{ \partial{|S|}} \right]_{|S| = \bar{N}} = 0
\end{equation}

Solving this equation for $ \alpha $ in the mean field approximation for the gaussian case yields :
\footnotesize
\begin{equation}
