...
\begin{figure}
\includegraphics[scale=.5]{./figures/coalitions/coalitions6.pdf}
...
Data were collected from [] (similar data can be found at [] for the United States). The variables used in the simulation are :
...
Power curves are functions that, for a given type of generator, map the input to the output power. For windturbines and solar arrays these functions are well studied and approximations have been proposed. For the wind turbines, the power curve is thus specified by 4 values :
...
The most interesting part is the increase of output power when the wind speed is in the cut-in-speed rated-output-speed range. Even if sometimes a simple linear model is used, the increase has been shown to be non linear and some more complex exponential fit can be found in the litterature.

The input quantity desired for our power curve model for solar arrays is a radiance in $ W.m^{-2} $, which can be difficult to find in weather station available data. As we mainly collected nebulosity series, we used the Helios model described in []. This model enabled us to compute perfect (clear blue sky situation) solar radiances at some specific locations on earth and at given timestamps. As nebulosity is a measure of the sky cloudiness, we can use the nebulosity series as degradation factors on the clear blue sky model (see [] for more details) :
...
data, helios model, power curves, scema
